...
<div  class="container d-flex align-items-center justify-content-center">
    <div class="card <?=   cook('color') ? cook('color') : 'bg-dark'; ?>" style="width: 18rem;">
        ...
        <div class="card-body">
            ...
            <?php
            if(get('islem')==null){
                echo '<div class="alert alert-success">Oturum Açıldı</div>';
            }elseif(get('islem')==false){
                echo '<div class="alert alert-danger">İşlem Başarısız</div>';
            }elseif(get('islem')==true){
                echo '<div class="alert alert-success">İşlem Başarılı</div>';
            }
            ?>
            <form action="islem.php?islem=hakkimda" method="post">
                <textarea class="form-control <?=   cook('color') ? cook('color') : 'bg-dark'; ?> text-primary" name="hakkimda" id="" cols="30" rows="10">
                <?= htmlspecialchars_decode($hakkimda) ?>
                </textarea>
